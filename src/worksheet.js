...
async function loadWorksheet(index) {
    ...
    document.getElementById("worksheet-number").innerHTML = `Arbeitsblatt ${index+1}`;
    document.getElementById("worksheet-title").innerHTML = `<h1>${data.title}</h1>`;
    document.getElementById("worksheet-description").innerHTML = data.description;
    if (data.image) {
        document.querySelector("#worksheet-image-wrapper").classList.remove("hide-element");
        document.getElementById("worksheet-image").innerHTML = `<img src="${data.image}" class="figure-img img-fluid" alt="${data.imageAlt}" >
            <figcaption class="figure-caption" id="worksheet-image-description">${data.imageDescription}</figcaption>`;
    } else {
        document.querySelector("#worksheet-image-wrapper").classList.add("hide-element");
    }

    // Worksheet code
    if (data.code) {
        document.querySelector("#worksheet-code-container").classList.remove("hide-element");
        // Code
        if (Object.keys(data.code).length === 1) {
            // if only one language version given, disable language buttons
            // TODO: Does not work yet
            document.getElementById("worksheet-code-language-buttons").innerHTML = `<button class="btn btn-sm btn-outline-secondary btn-disabled" disabled>${languageNames[data.primaryLanguage]}</button>`
        } else {
            let languageButtonsHtml = "";
            for (const language in data.code) {
                if (language === currentSelectedLanguage || (currentSelectedLanguage === "" && language === data.primaryLanguage)) {
            ...
            document.getElementById("worksheet-code-language-buttons").innerHTML = languageButtonsHtml;
        }
        if(currentSelectedLanguage === "") {
            currentSelectedLanguage = data.primaryLanguage;
        }
        document.getElementById("worksheet-code").innerHTML = `<code class="language-${data.primaryLanguage}">${data.code[currentSelectedLanguage]}</code>`;
        ...
        document.querySelector("#worksheet-code-container").classList.add("hide-element");
    }
    ...
    let citationsHtml = "";
    if(data.citations) {
        document.querySelector("#worksheet-citations-wrapper").classList.remove("hide-element");
        ...
        document.querySelector("#worksheet-citations-wrapper").classList.add("hide-element");
    }
}

function getCodeHelpers(index, data, language) {
    if (data.codeHelpers[language]) {
        document.querySelector("#code-helpers-wrapper").classList.remove("hide-element");
        let codeHelpersHtml = "";
        ...
        document.querySelector("#code-helpers-wrapper").classList.add("hide-element");
    }
}